...
#include <stdio.h>
#define ALIGNMENT                                                              \
  16 // Common alignment for safety (adjust to 8 or 32 if needed for your
     // platform/types)
...
typedef struct {
  void *(*alloc)(
      void *context,
      size_t size); // Allocates 'size' bytes, returns NULL on failure
  void (*free)(
      void *context,
      void *ptr); // Frees the pointer (optional, can be NULL for arenas)
  void *context;  // Allocator-specific data (e.g., arena pointer)
} MemoryAllocator;

#define ALLOC_DA(allocator, da, type, cap)                                     \
  do {                                                                         \
    da.items =                                                                 \
        (type *)allocator.alloc(allocator.context, sizeof(type) * (cap));      \
    if (!da.items)                                                             \
      return NULL;                                                             \
    da.capacity = (cap);                                                       \
    da.count = 0;                                                              \
...
static inline void arena_init(memory_arena *arena, mem_base *base,
...
// Internal
static inline void *_PushStruct(memory_arena *arena, size_t size) {
...
static inline void *arena_alloc(void *context, size_t size) {
  ...
  if (arena->used + align_size(size) > arena->size) {
    printf(
        "arena_alloc: Out of memory! Requested=%zu (aligned), Available=%zu\n",
