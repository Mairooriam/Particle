...
camera.projection = CAMERA_ORTHOGRAPHIC;
  ...
  while (!WindowShouldClose()) {
    ...
    if (IsKeyDown(KEY_RIGHT) || IsKeyDown(KEY_D)) input.camera.position.x += panSpeed;
    if (IsKeyDown(KEY_LEFT)  || IsKeyDown(KEY_A)) input.camera.position.x -= panSpeed;
    if (IsKeyDown(KEY_UP)    || IsKeyDown(KEY_W)) input.camera.position.y += panSpeed;
    if (IsKeyDown(KEY_DOWN)  || IsKeyDown(KEY_S)) input.camera.position.y -= panSpeed;
    ...
    if (wheel != 0) {
        ...
        if (input.camera.fovy < 5) input.camera.fovy = 5;
        if (input.camera.fovy > 1000) input.camera.fovy = 1000;
    }
