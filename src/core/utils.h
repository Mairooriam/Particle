...
#include "stdlib.h"
  ...
  #define TIMING_FRAME_BEGIN() do { g_timing_count = 0; } while(0)
    ...
    do { \
      ...
      if (g_timing_accumulated_time >= g_timing_print_interval) { \
        for (int i = 0; i < g_timing_count; i++) { \
          printf("%s: %.3f ms\n", g_timing_entries[i].name, g_timing_entries[i].total_ms); \
        } \
  ...
  #define TIMING_SET_INTERVAL(seconds) do { g_timing_print_interval = seconds; } while(0)
#else
  #define TIME_IT(label, code) do { code; } while (0)
